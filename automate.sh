...
sudo apt install php8.2-curl php8.2-dom php8.2-mbstring php8.2-xml php8.2-mysql$
#enable rewrite
...
sudo mysql -uroot -e "CREATE DATABASE goodydb;"
sudo mysql -uroot -e "CREATE USER 'coder29'@'localhost' IDENTIFIED BY 'secured2$
sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON goodydb.* TO 'coder29'@'localhost$
...
sudo sed -i '25 s/=laravel/=goodydb/' /var/www/laravel/.env
sudo sed -i '26 s/=root/=coder29/' /var/www/laravel/.env
sudo sed -i '27 s/=/=secured23/' /var/www/laravel/.env
